...
# Soft/runtime deps (warn only — OCR is optional; PDF needs poppler)
if ! ldconfig -p 2>/dev/null | grep -q 'libpoppler-cpp.so'; then
  if [[ ! -e /usr/lib/x86_64-linux-gnu/libpoppler-cpp.so.0 ]]; then
    echo "warn: libpoppler-cpp.so.0 not found — PDF ingest will fail to link/run" >&2
    echo "      apt install libpoppler-cpp0v5" >&2
  fi
fi
...
echo "  deps: libpoppler-cpp0v5 (PDF); optional OCR: libtesseract4 tesseract-ocr-eng"
